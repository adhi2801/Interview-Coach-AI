...
const PREFERENCE_ROWS = [
  ...
  { key: "live_coaching_telemetry", label: "Live coaching", description: "Show WPM and real-time confidence scores during interview." },
...
export default function Settings({ user, onLogout, onGoBack, onProfileUpdate }) {
  ...
  const getGradient = (name) => {
    if (!name) return "linear-gradient(135deg, #3b82f6, #2997FF)";
    const colors = [
      "linear-gradient(135deg, #3b82f6, #2997FF)",
      "linear-gradient(135deg, #10b981, #3b82f6)",
      "linear-gradient(135deg, #f59e0b, #ef4444)",
      "linear-gradient(135deg, #ec4899, #2997FF)"
    ];
  ...
  const tabs = [
    ...
    { id: "telemetry", label: "Rating & stats", icon: Activity },
    { id: "hardware", label: "Audio & Hardware", icon: Mic },
    { id: "danger", label: "Danger Zone", icon: ShieldAlert, danger: true }
  ];

  return (
    <div className="min-h-screen bg-canvas text-label font-sans selection:bg-accent/30 overflow-x-hidden relative flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-16 border-b border-hairline bg-canvas/60 backdrop-blur-2xl flex items-center justify-between px-6 lg:px-12 flex-shrink-0">
        <button
          onClick={onGoBack}
          className="flex items-center gap-2 text-caption font-medium text-label-2 hover:text-white transition-colors bg-white/[0.02] border border-hairline px-4 py-2 rounded-lg hover:bg-white/[0.05] outline-none"
        >
          <ArrowLeft size={14} /> Dashboard
        </button>

        <div className="hidden sm:flex items-center gap-2 bg-surface border border-hairline px-3.5 py-1.5 rounded-xl text-xs font-medium text-label-2 shadow-inner">
          <Search size={14} /> Search Settings
          <kbd className="ml-2 tabular-nums text-[10px] bg-white/10 px-1.5 py-0.5 rounded text-label-2">⌘K</kbd>
        </div>
      </header>
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-colors outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                      isActive
                        ? tab.danger ? "text-rose-400" : "text-white"
                        : tab.danger ? "text-rose-500/70 hover:text-rose-400 hover:bg-rose-500/10" : "text-label-2 hover:text-label hover:bg-white/[0.02]"
                    }`}
                  >
                    {isActive && (
                      <motion.div
                    ...
                    {tab.id === "telemetry" && profile?.elo_rating != null && (
                      <span className="relative z-10 ml-auto text-[10px] tabular-nums font-bold text-accent bg-accent/15 border border-accent/25 px-1.5 py-0.5 rounded-full tabular-nums">
          ...
          <AnimatePresence mode="wait">
            ...
            >
                  ...
                  <GlassCard className="p-8">
                    {profileLoading ? (
                      ...
                      <div className="flex flex-col items-center text-center gap-2 py-6">
                        ...
                        <p className="text-sm text-label-2">Couldn't load your profile.</p>
                        <button onClick={fetchProfile} className="text-xs font-bold text-accent hover:text-accent flex items-center gap-1">
                          <RotateCcw size={11} /> Retry
                        </button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-6 mb-8 pb-8 border-b border-hairline">
                        <div
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            {editingName ? (
                              <>
                                <input
                                  ...
                                  className="text-2xl font-extrabold tracking-tight text-white bg-white/[0.05] border border-accent/40 rounded-lg px-2 py-1 outline-none min-w-0"
                                />
                                ...
                                <button onClick={() => setEditingName(false)} className="p-1.5 rounded-lg bg-white/5 border border-hairline text-label-2 hover:text-white">
                                  <X size={14} />
                                </button>
                              </>
                            ) : (
                              <>
                                ...
                                <button onClick={startEditingName} className="p-1.5 rounded-lg text-label-3 hover:text-white hover:bg-white/[0.06] transition-colors">
                                  <Pencil size={13} />
                                </button>
                              </>
                            )}
                            <AnimatePresence>
                              {nameSaveState !== "idle" && (
                                <motion.span
                                  ...
                                  className={`text-[10px] tabular-nums font-bold flex items-center gap-1 ${nameSaveState === "error" ? "text-rose-400" : "text-emerald-400"}`}
                                >
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            {profile?.bracket ? (
                              <span className="text-caption font-medium text-label-2 bg-white/5 border border-hairline px-2.5 py-0.5 rounded-full">
                                {profile.bracket.label} &middot; {profile.bracket.role}
                              </span>
                            ) : (
                              <span className="text-caption font-medium text-label-3 bg-white/[0.03] border border-hairline px-2.5 py-0.5 rounded-full italic">
                                No bracket yet — complete a session
                              </span>
                            )}
                          </div>
                        </div>
                        <div className="hidden sm:flex items-center gap-5 pl-5 border-l border-hairline shrink-0">
                          <div className="text-center">
                            <div className="text-xl font-extrabold text-white tabular-nums">{Math.round(profile?.elo_rating ?? 1200)}</div>
                            <div className="text-caption font-medium text-label-3 mt-0.5">ELO</div>
                          </div>
                          <div className="text-center">
                            <div className="text-xl font-extrabold text-white tabular-nums">{profile?.total_sessions ?? 0}</div>
                            <div className="text-caption font-medium text-label-3 mt-0.5">Sessions</div>
                          </div>
                          <div className="text-center">
                            <div className="text-xl font-extrabold text-white tabular-nums">
                              {profile?.avg_score != null ? profile.avg_score.toFixed(1) : "—"}
                            </div>
                            <div className="text-caption font-medium text-label-3 mt-0.5">Avg Score</div>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-1 tabular-nums text-xs">
                      <ConfigRow
                        ...
                        managedNote="Used to sign in; can't be changed"
                      />
                      <ConfigRow
                        label="Password"
                        value={<span className="flex items-center gap-2 text-label-2"><Key size={14} /> Stored as a bcrypt hash, never in plain text</span>}
                        managedNote="Password changes aren't available yet"
                      />
                    </div>
                  </GlassCard>

                  <GlassCard className="p-8">
                    <h3 className="text-xs font-medium text-label-2 mb-6 flex items-center gap-2">
                      <Settings2 size={16} className="text-label-2" /> Interview Preferences
                    </h3>
              ...
              {activeTab === "telemetry" && (
                <GlassCard className="p-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    <div className="bg-surface border border-hairline p-6 rounded-2xl shadow-inner">
                      <span className="block text-caption font-medium text-label-3 mb-2">Current Rating</span>
                      <span className="text-4xl font-extrabold text-white tabular-nums tracking-tighter tabular-nums">
                        {Math.round(profile?.elo_rating ?? 1200)}
                      </span>
                    </div>
                    <div className="bg-surface border border-hairline p-6 rounded-2xl shadow-inner">
                      <span className="block text-caption font-medium text-label-3 mb-2">Total Sessions</span>
                      <span className="text-4xl font-extrabold text-white tabular-nums tracking-tighter tabular-nums">
                        {profile?.total_sessions ?? 0}
                      </span>
                    </div>
                    <div className="bg-surface border border-hairline p-6 rounded-2xl shadow-inner">
                      <span className="block text-caption font-medium text-label-3 mb-2">Average Score</span>
                      <span className="text-4xl font-extrabold text-emerald-400 tabular-nums tracking-tighter tabular-nums">
                        {profile?.avg_score != null ? profile.avg_score.toFixed(1) : "—"}
                      </span>
                    </div>
                  </div>
                  ...
                  {profile?.bracket ? (
                    <div className="bg-accent/[0.06] border border-accent/20 p-5 rounded-2xl mb-6">
                      <span className="block text-caption font-medium text-accent mb-2">Current Bracket</span>
                    ...
                    <div className="bg-white/[0.03] border border-hairline p-5 rounded-2xl mb-6">
                      <p className="text-sm text-label-3">No bracket yet — this appears once you complete a session in a tracked role.</p>
                    </div>
                  )}

                  <p className="text-xs text-label-2 font-medium leading-relaxed bg-blue-500/5 border border-blue-500/10 p-4 rounded-xl">
              ...
              {activeTab === "hardware" && (
                <GlassCard className="p-8">
                  <h3 className="text-xs font-medium text-label-2 mb-6 flex items-center gap-2">
                    <Mic size={16} className="text-accent" /> Audio Configuration
                  </h3>

                  <div className="space-y-6">
                    <div>
                      <label className="text-caption font-medium text-label-3 block mb-2">Input Device Selector</label>
                      {devicesLoading ? (
                        ...
                        <select
                          ...
                          className="w-full bg-surface border border-hairline rounded-xl px-4 py-3 text-sm font-semibold text-white outline-none focus:border-accent appearance-none shadow-inner cursor-pointer"
                        >
                          {devices.map((d, i) => (
                        ...
                        <p className="text-xs text-label-3">No input devices found, or permission hasn't been granted yet — click "Test Microphone" below to request access.</p>
                      )}
                    </div>

                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <label className="text-caption font-medium text-label-3 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-lg border transition-colors ${
                            micTesting ? "bg-rose-500/10 border-rose-500/30 text-rose-400" : "bg-white/[0.04] border-hairline text-label-2 hover:text-white"
                          }`}
                        >
                      ...
                      <div className="flex items-end gap-1 h-12 w-full p-2.5 bg-surface border border-hairline rounded-xl shadow-inner overflow-hidden">
                        {levels.map((h, i) => (
                          <div
                            key={i}
                      ...
                      <p className="text-[10px] text-label-3 mt-2">
              ...
              {activeTab === "danger" && (
                <GlassCard className="p-8 border-rose-500/20 bg-rose-950/10">
                  <h3 className="text-xs font-medium text-rose-400 mb-6 flex items-center gap-2">
                  ...
                  <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl border border-rose-500/10 bg-rose-500/5">
                      <div>
                        ...
                        <p className="text-xs font-medium text-label-2">Log out of your account on this device. Your data will remain safe.</p>
                      </div>
                      <button
                        onClick={onLogout}
                        className="flex-shrink-0 flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-white/[0.04] border border-hairline text-white text-sm font-bold hover:bg-white/10 active:scale-95 transition-all w-full sm:w-auto outline-none"
                      >
                        ...
                        <div className="w-full">
                          ...
                          <div className="flex flex-col sm:flex-row gap-3">
                            <button
                              ...
                              disabled={deleting}
                              className="flex-1 px-5 py-2.5 rounded-xl border border-hairline text-label-2 text-sm font-bold hover:bg-white/[0.05] transition-all outline-none"
                            >
                              Cancel
                            </button>
                            <button
...
function GlassCard({ children, className = "" }) {
  ...
  return (
    <div
      ...
      className={`relative rounded-3xl bg-surface border border-hairline overflow-hidden backdrop-blur-2xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08),_0_20px_40px_-10px_rgba(0,0,0,0.8)] ${className}`}
    >
      <div
...
function ConfigRow({ label, value, managedNote }) {
  return (
    <div className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.02] transition-colors">
      <div>
        <p className="text-caption font-medium text-label-3 mb-1">{label}</p>
        <div className="text-sm font-semibold text-white">{value}</div>
      </div>
      <span className="text-caption text-label-3 text-right max-w-[180px]">{managedNote}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.02] transition-colors cursor-pointer" onClick={onToggle}>
      <div className="pr-4">
          ...
          <AnimatePresence>
            {saveState && (
              <motion.span
                ...
                className={`text-[9.5px] tabular-nums font-bold flex items-center gap-1 ${saveState === "error" ? "text-rose-400" : "text-emerald-400"}`}
              >
        ...
        <p className="text-xs font-medium text-label-3">{description}</p>
      </div>

      <div className={`relative w-11 h-6 flex items-center rounded-full p-1 transition-colors duration-200 ease-in-out shrink-0 ${isOn ? 'bg-accent' : 'bg-white/10'}`}>
        <motion.div
          layout
          initial={false}
